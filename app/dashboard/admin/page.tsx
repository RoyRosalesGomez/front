...
export default function AdminDashboard() {
  ...
  async function handleSaveUser() {
    ...
    const confirmed = await alert.confirm(
      "¿Guardar cambios?",
      `Se actualizará la información del usuario ${showUserDetails.name} ${showUserDetails.lastName}`
    );
    ...
    try {
      ...
      await alert.error("Error", e?.message || "No se pudo actualizar");
    }
  }
  ...
  const handleActivateUser = async (userId: number) => {
    ...
    try {
      ...
      await alert.error("Error", "No se pudo activar el usuario");
    }
  };
  ...
  const handleDeactivateUser = async (userId: number) => {
    ...
    try {
      ...
      await alert.error("Error", "No se pudo desactivar el usuario");
    }
  };
  ...
  const handleChangePassword = async () => {
    ...
    const confirmed = await alert.confirm(
      "¿Cambiar contraseña?",
      `Se cambiará la contraseña del usuario ${showChangePassword.name} ${showChangePassword.lastName}`
    );
    ...
    try {
      ...
      await alert.error("Error", "No se pudo cambiar la contraseña");
    }
  };
  ...
  const handleApproveProduct = async (productId: number) => {
    const confirmed = await alert.confirm(
      "¿Aprobar producto?",
      "El producto estará disponible en el marketplace para todos los clientes."
    );
    ...
    try {
      ...
      await alert.success(
        "Producto aprobado",
        "El producto está ahora disponible en el marketplace"
      );
      await loadProducts();
      await loadStatistics();
      await loadActivities();
      ...
      await alert.error("Error", "No se pudo aprobar el producto");
    }
  };
  ...
  const handleRejectProduct = async (productId: number) => {
    const confirmed = await alert.confirm(
      "¿Rechazar producto?",
      "El producto no estará disponible en el marketplace."
    );
    ...
    try {
      ...
      await alert.error("Error", "No se pudo rechazar el producto");
    }
  };

  // Funciones de gestión de agro veterinarias
  const handleToggleVetShop = async (vetShopId: number) => {
    const shop = vetShops.find((v) => v.id === vetShopId);
    const action = shop?.active ? "desactivar" : "activar";

    const confirmed = await alert.confirm(
      `¿${action.charAt(0).toUpperCase() + action.slice(1)} agro veterinaria?`,
      `La agro veterinaria será ${action}da.`
    );
    ...
    try {
      ...
      await alert.success(
        "Estado actualizado",
        `Agro veterinaria ${action}da exitosamente`
      );
      await loadVetShops();
      await loadStatistics();
      await loadActivities();
      ...
      await alert.error("Error", "No se pudo actualizar la agro veterinaria");
    }
  };
  ...
  const handleAddVetShop = async (e: React.FormEvent) => {
    ...
    const confirmed = await alert.confirm(
      "¿Crear agro veterinaria?",
      `Se creará la agro veterinaria "${vetShopForm.name}"`
    );
    ...
    try {
      ...
      await loadActivities();
      await alert.success(
        "Agro veterinaria creada",
        "La agro veterinaria ha sido agregada exitosamente"
      );
      ...
      await alert.error("Error", "No se pudo crear la agro veterinaria");
    }
  };
                    ...
                    {vetShopForm.image && (
                      <div className="relative mt-2">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            setVetShopForm({ ...vetShopForm, image: "" })
                          }
                  ...
                  {editVetForm.image && (
                    <div className="relative mt-2">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setEditVetForm({ ...editVetForm, image: "" })
                        }
                        className="absolute top-2 right-2 bg-red-500 text-white p-2 rounded-full hover:bg-red-600"
                      >
                        <X className="h-4 w-4" />
                ...
                <div className="flex gap-4 mt-8">
                  <Button
                    ...
                    onClick={async () => {
                      const confirmed = await alert.confirm(
                        "¿Guardar cambios?",
                        `Se actualizará la información de "${editVetForm.name}"`
                      );
